...
contract Template is BaseTemplate, TokenCache {
    ...
    */
    ...
    // Next we install and create permissions for the tasks app
    //--------------------------------------------------------------//
    function _setupCustomApp(
        ...
        internal
    {
        Tasks app = _installTasks(_dao);
        _createTasksPermissions(_acl, app, _voting, _voting);
    }

    function _installTasks(
        Kernel _dao
    )
        internal returns (Tasks)
    {
        bytes32 _appId = keccak256(abi.encodePacked(apmNamehash("open"), keccak256("tasks")));
        bytes memory initializeData = abi.encodeWithSelector(Tasks(0).initialize.selector);
        return Tasks(_installDefaultApp(_dao, _appId, initializeData));
    }

    function _createTasksPermissions(
        ACL _acl,
        Tasks _app,
        address _grantee,
        address _manager
        ...
        internal
    {
        _acl.createPermission(ANY_ENTITY, _app, _app.CREATE_TASK_ROLE(), _manager);
        _acl.createPermission(ANY_ENTITY, _app, _app.CHANGE_PRIORITY_ROLE(), _manager);
        _acl.createPermission(ANY_ENTITY, _app, _app.COMPLETE_TASK_ROLE(), _manager);
        _acl.createPermission(ANY_ENTITY, _app, _app.DELETE_TASK_ROLE(), _manager);
        _app.createTask("Comprar Leche", Tasks.Priority.HIGH);
        _app.createTask("Comprar Cafe", Tasks.Priority.MEDIUM);
        _app.createTask("Corregir examenes", Tasks.Priority.LOW);
        _app.changePriority(2, Tasks.Priority.HIGH);
        _app.completeTask(0);
        _app.completeTask(1);
        _app.deleteTask(0);
    }
